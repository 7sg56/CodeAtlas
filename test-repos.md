# CodeAtlas Testing Suite

## 📦 Test Repositories

### Spring Boot & Java
- [Spring PetClinic](https://github.com/spring-petclinic/spring-petclinic-reactjs)
- [Fullstack Boilerplate](https://github.com/DevSkillsHQ/fullstack-boilerplate-java-springboot-react-typescript)
- [React-Spring CRUD](https://github.com/RameshMF/ReactJS-Spring-Boot-CRUD-Full-Stack-App)
- [React and Spring](https://github.com/kantega/react-and-spring)
- [Dockerized Spring/React](https://github.com/jhordyess/dockerized-spring-react-mysql)
- [In28Minutes Examples](https://github.com/in28minutes/spring-boot-react-fullstack-examples)

### Node.js & MERN
- [MERN E-Commerce](https://github.com/HuXn-WebDev/MERN-E-Commerce-Store)
- [MERN Stack Example](https://github.com/mongodb-developer/mern-stack-example)
- [MERN Starter](https://github.com/joshuaslate/mern-starter)
- [MERN CRUD](https://github.com/MiladJoodi/MERN_Stack_CRUD)
- [Node-Express-Postgres](https://github.com/japananh/node-express-postgres-boilerplate)
- [Simple React Full Stack](https://github.com/crsandeep/simple-react-full-stack)
- [Clean Architecture Node](https://github.com/panagiop/node.js-clean-architecture)

### Go
- [Go Gin RealWorld App](https://github.com/gothinkster/golang-gin-realworld-example-app)
- [Go Echo RealWorld App](https://github.com/gothinkster/echo-realworld-example-app)
- [Gin Examples Collection](https://github.com/gin-gonic/examples)

### Ruby & Rails
- [Rails RealWorld App](https://github.com/gothinkster/rails-realworld-example-app)
- [Discourse (Large Codebase)](https://github.com/discourse/discourse)
- [Lobsters](https://github.com/lobsters/lobsters)

### PHP & Laravel/Symfony
- [Laravel RealWorld App](https://github.com/gothinkster/laravel-realworld-example-app)
- [Symfony Demo Application](https://github.com/symfony/demo)
- [BookStack](https://github.com/BookStackApp/BookStack)

### Rust
- [Rust Actix RealWorld App](https://github.com/gothinkster/rust-actixweb-realworld-example-app)
- [Rust Rocket RealWorld App](https://github.com/gothinkster/rust-rocket-realworld-example-app)
- [Actix Examples Collection](https://github.com/actix/examples)

## ✅ Verification Checklist
- [ ] Framework tags & counters correct
- [ ] Language percentage breakdown accurate
- [ ] Entrypoints & Server bootstrap identified
- [ ] API routes method, path, and file links correct
- [ ] Architecture Overview tree logical (Server -> Entrypoint -> API Routes)

## 📝 Analysis Notes
- **Issue #001:** Duplicate Spring Boot tags identified in `ANIMAL-MERGE-GAME` analyze view.
